...
const ProjectCard = ({ project, aosDelay }) => (
  <div 
    ...
    className={`relative rounded-2xl p-[1px] group transition-all duration-500 ${
      project.isFlagship 
        ? 'bg-gradient-to-br from-[#4F7CFF]/50 via-white/10 to-[#7C8CFF]/30 hover:from-[#4F7CFF] hover:via-[#7C8CFF]/30 hover:to-[#4F7CFF]/60' 
  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 ${
      ...
      {/* Badge */}
      {project.badge && (
        <span className="inline-block text-xs font-bold tracking-widest uppercase text-[#7C8CFF] bg-[#4F7CFF]/10 px-3 py-1 rounded-full border border-[#4F7CFF]/20 mb-4">
          {project.badge}
        </span>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {project.techTags.map((tag) => (
          <span 
            key={tag}
            className="px-3 py-1 text-xs font-bold text-white/70 bg-white/5 rounded-full border border-white/10 hover:bg-[#4F7CFF]/20 hover:border-[#4F7CFF]/30 hover:text-[#7C8CFF] transition-all duration-300 cursor-default"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {project.links.demo !== undefined && (
          <a 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${
              project.links.demo 
                ? 'bg-[#1d4ed8] text-white hover:bg-[#1e40af] hover:shadow-[0_0_20px_rgba(29,78,216,0.4)]' 
        ...
        {project.links.frontendDemo && (
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#1d4ed8] text-white text-sm font-semibold hover:bg-[#1e40af] hover:shadow-[0_0_20px_rgba(29,78,216,0.4)] transition-all duration-300"
          >
            <ExternalLinkIcon />
            Frontend Demo
          </a>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div data-aos="fade-up" className="mb-16 md:mb-20">
          ...
          <p className="text-white/50 text-base md:text-lg max-w-lg font-medium leading-relaxed">
            Real-time machine learning applications and IoT telemetry systems engineered with Python and embedded microcontrollers.
          </p>
        </div>
